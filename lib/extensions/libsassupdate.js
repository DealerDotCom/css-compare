...
const removeMiscProperties = (rule) => {
  if (rule.declarations && rule.declarations.length) {
    rule.declarations = rule.declarations.filter((declaration) => {
      const prop = declaration.property;
      const val = declaration.value;
  ...
  // Recurse
  if (rule.rules && rule.rules.length) {
    rule.rules.forEach(removeMiscProperties);
  }
  if (rule.keyframes && rule.keyframes.length) {
    rule.keyframes.forEach(removeMiscProperties);
  }
};
...
module.exports = function(root, rw) {
  ...
  root.rules.forEach(removeMiscProperties);
